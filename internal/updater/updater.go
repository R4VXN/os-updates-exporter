...
func (u *Updater) Check(ctx context.Context) (CheckResult, error) {
	...
	return CheckResult{
		Current:         u.current,
		Latest:          latest,
		UpdateAvailable: normalizeVer(latest) != normalizeVer(u.current),
	}, nil
}
...
func (u *Updater) Run(ctx context.Context) (RunResult, error) {
	...
	newBin := filepath.Join(tmpDir, "os-updates-exporter.new")
...
func (u *Updater) fetchLatestVersion(ctx context.Context) (string, error) {
	...
	if data.TagName == "" {
		return "", errors.New("missing tag_name in github response")
	}
